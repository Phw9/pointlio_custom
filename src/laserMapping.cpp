...
std::cout<<"num point : " << meas.lidar->points.size() << std::endl; 
    ...
    if(lidar_type == RS)
    {
        if (scan_pub_en)
        {
            ...
            for (int i = 0; i < size; i++)
            {
                // if (i % 3 == 0)
                // {
                laserCloudWorld->points[i].x = feats_down_world->points[i].z;
                laserCloudWorld->points[i].y = feats_down_world->points[i].y;
                laserCloudWorld->points[i].z = feats_down_world->points[i].x;
...
void set_posestamp(T & out)
{
    if (!use_imu_as_input)
    {
        if(lidar_type == RS)
        {
            out.position.x = kf_output.x_.pos(2);
            out.position.y = kf_output.x_.pos(1);
            out.position.z = kf_output.x_.pos(0);
            out.orientation.x = kf_output.x_.rot.coeffs()[2];
            out.orientation.y = kf_output.x_.rot.coeffs()[1];
            out.orientation.z = kf_output.x_.rot.coeffs()[0];
...
int main(int argc, char** argv)
{
    ...
    kf_input.init_dyn_share_modified(get_f_input, df_dx_input, h_model_input);  // use match_s param
...
//------------------------------------------------------------------------------------------------------
    ...
    // For hilti 2023 save trajectory
    if(traj_save_en)
    {
